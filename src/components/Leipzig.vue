<template>
    <div>
        <div class="example gloss--glossed">
            <p v-if="gloss.original" class="gloss__line--original gloss__line gloss__line--0">
                {{ gloss.original }}
            </p>
            <div class="gloss__words">
                <div class="gloss__word" v-for="idx in tokenizeGlossLength[0].length" :key="idx">
                    <p v-for="(line, i) in tokenizeGloss" :key="i" :class="`gloss__line gloss__line--${i+1}`">
                        
                        <template v-if="line[idx-1].length > 1"> 
                            <template v-for="(tag, j) in line[idx-1]">
                                <!-- if split by '-' -->
                                <template v-if="RegExp('[A-Z1-3]+-[A-Z1-3]+').test(tag)">
                                    <template v-for="(sub, k) in tag.split('-')">
                                        <abbr class="gloss__abbr" :title="abbr[sub]" :key="sub + j + k">{{ sub }}</abbr>
                                        <template v-if="k + 1 < tag.split('-').length">-</template>
                                    </template>
                                </template>
                                <template v-else>
                                    <abbr class="gloss__abbr" :title="abbr[tag]" :key="tag + j">{{ tag }}</abbr>
                                </template>
                                        
                                <template v-if="j + 1 < line[idx-1].length">.</template>
                            </template>
            ...
            <p v-for="(line, i) in gloss.free" :key="line" :class="`gloss__line--free gloss__line gloss__line--${tokenizeGlossLength[0] + i}`">
                {{ line }}
            </p>
...
export default {
    computed: {
        tokenizeGloss: function() {
            ...
            .map(
                line => line.match(/{(.*?)}|([^\s]+)/g)
                            .map(token => token.replace(/^{/, '').replace(/}$/, '').split('.')
                )
            )
        },
        tokenizeGlossLength: function() {
            ...
            .map(
                line => line.match(/{(.*?)}|([^\s]+)/g)
                            .map(token => token.replace(/^{/, '').replace(/}$/, '').split('.').length 
                )
            )
        },  
